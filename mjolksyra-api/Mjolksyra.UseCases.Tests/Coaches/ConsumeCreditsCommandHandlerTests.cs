...
public class ConsumeCreditsCommandHandlerTests
{
    ...
    private static CreditActionPricing BuildPricing(int cost) => new()
    {
        Id = Guid.NewGuid(),
        Action = CreditAction.PlanWorkout,
        CreditCost = cost,
    };

    private static UserCredits BuildCredits(Guid coachId, int included, int purchased, int version = 0) => new()
    {
        Id = Guid.NewGuid(),
        CoachUserId = coachId,
    ...
    public async Task Handle_InsufficientCredits_ReturnsError_NoAtomicDeduct()
    {
        ...
        pricingRepo.Setup(r => r.GetByAction(CreditAction.PlanWorkout, It.IsAny<CancellationToken>()))
        ...
        var result = await handler.Handle(new ConsumeCreditsCommand(coachId, CreditAction.PlanWorkout), default);
        ...
        ledgerRepo.Verify(r => r.Append(It.IsAny<CreditLedger>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task Handle_SpillsIntoPurchased_DeductsSplit()
    {
        ...
        pricingRepo.Setup(r => r.GetByAction(CreditAction.GenerateBlock, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new CreditActionPricing { Id = Guid.NewGuid(), Action = CreditAction.GenerateBlock, CreditCost = 5 });
        ...
        var handler = CreateHandler(creditsRepo, pricingRepo, ledgerRepo);
        var result = await handler.Handle(new ConsumeCreditsCommand(coachId, CreditAction.GenerateBlock), default);
    ...
    public async Task Handle_Success_WritesLedgerEntryWithCorrectValues()
    {
        ...
        pricingRepo.Setup(r => r.GetByAction(CreditAction.AnalyzeWorkoutText, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new CreditActionPricing { Id = Guid.NewGuid(), Action = CreditAction.AnalyzeWorkoutText, CreditCost = 1 });
        ...
        CreditLedger? capturedEntry = null;
        ledgerRepo.Setup(r => r.Append(It.IsAny<CreditLedger>(), It.IsAny<CancellationToken>()))
            .Callback<CreditLedger, CancellationToken>((e, _) => capturedEntry = e)
            .Returns(Task.CompletedTask);

        var handler = CreateHandler(creditsRepo, pricingRepo, ledgerRepo);
        await handler.Handle(new ConsumeCreditsCommand(coachId, CreditAction.AnalyzeWorkoutText, referenceId), default);

        Assert.NotNull(capturedEntry);
        Assert.Equal(coachId, capturedEntry!.CoachUserId);
        Assert.Equal(CreditAction.AnalyzeWorkoutText, capturedEntry.Action);
        Assert.Equal(CreditLedgerType.Deduct, capturedEntry.Type);
