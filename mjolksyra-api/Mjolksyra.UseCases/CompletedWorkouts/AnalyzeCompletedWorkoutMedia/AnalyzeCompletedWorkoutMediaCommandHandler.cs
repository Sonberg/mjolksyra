...
public class AnalyzeCompletedWorkoutMediaCommandHandler(
    IMediator mediator,
    ...
    IWorkoutMediaAnalysisAgent workoutMediaAnalysisAgent)
    : IRequestHandler<AnalyzeCompletedWorkoutMediaCommand, OneOf<CompletedWorkoutMediaAnalysisResponse, AnalyzeCompletedWorkoutMediaForbidden, AnalyzeCompletedWorkoutMediaInsufficientCredits>>
{
    public async Task<OneOf<CompletedWorkoutMediaAnalysisResponse, AnalyzeCompletedWorkoutMediaForbidden, AnalyzeCompletedWorkoutMediaInsufficientCredits>> Handle(AnalyzeCompletedWorkoutMediaCommand request, CancellationToken cancellationToken)
    {
        ...
        var chatMessages = await completedWorkoutChatMessageRepository.GetByWorkoutId(
            request.TraineeId,
            request.CompletedWorkoutId,
            cancellationToken);

        var sessionExercises = completedWorkout.Exercises ?? (ICollection<CompletedExercise>)[];
        ...
        var workoutMedia = completedWorkout.Media
            .Where(m => !string.IsNullOrWhiteSpace(m.CompressedUrl ?? m.RawUrl))
            .Select(m => new
            {
                Url = m.CompressedUrl ?? m.RawUrl!,
                m.Type,
            });

        var chatMedia = chatMessages
