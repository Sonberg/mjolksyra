import { cn } from "@/lib/utils";

export const shellSegmentedContainerClass =
  "inline-flex items-stretch border-2 border-[var(--shell-border)] bg-[var(--shell-surface)] divide-x-2 divide-[var(--shell-border)]";

export function shellRoleLinkClass(isActive: boolean) {
  return cn(
    "inline-flex items-center justify-center rounded-none px-5 py-2 text-xs font-semibold outline-none transition-colors duration-150 md:text-sm",
    isActive
      ? "bg-[var(--shell-ink)] text-[var(--shell-surface)]"
      : "bg-[var(--shell-surface)] text-[var(--shell-ink)] hover:bg-[var(--shell-surface-strong)]",
  );
}

export function shellSectionTabClass(isActive: boolean) {
  return cn(
    "inline-flex h-10 items-center justify-center rounded-none px-4 py-2 text-sm font-semibold transition-colors",
    isActive
      ? "bg-[var(--shell-ink)] text-[var(--shell-surface)]"
      : "bg-[var(--shell-surface)] text-[var(--shell-muted)] hover:bg-[var(--shell-surface-strong)] hover:text-[var(--shell-ink)]",
  );
}
